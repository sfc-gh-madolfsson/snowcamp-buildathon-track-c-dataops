...
| Skill | Reach for it when you want to... |
|---|---|
| `/machine-learning` | The whole ML path — feature engineering, clustering, Model Registry, evaluation |
| `/snowpark-python` | Write the patient-level feature transforms as Snowpark DataFrames at scale |
| `/snowflake-notebooks` | Author the Snowflake notebook you build the model in |
| `/data-quality` | Quantify and clean the planted issues, and surface the metrics for the app |
| `/data-governance` | Classify the patient PII and keep it out of the model and the app |
